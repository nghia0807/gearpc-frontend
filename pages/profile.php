...
{
    $ch = curl_init("http://phpbe_app_service:5000/api/auth/me");
...
{
    $apiUrl = "http://phpbe_app_service:5000/api/orders/user?pageIndex=0&pageSize=1";
    $ch = curl_init($apiUrl);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_profile'])) {
        ...
        if ($isValid && ($hasFullNameChange || $hasEmailChange || $hasPasswordChange)) {
            ...
            // Send API request
            $ch = curl_init("http://phpbe_app_service:5000/api/auth/me/update");
