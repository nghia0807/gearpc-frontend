...
$pageSize = 10;
$apiUrl = $api . "/api/gifts/get?pageIndex=$pageIndex&pageSize=$pageSize";
$alerts = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_gift'])) {
    ...
    if ($code === '' || $name === '') {
        ...
        $ch = curl_init($api . '/api/gifts/add');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_gift'])) {
    ...
    if ($id === '' || $name === '') {
        ...
        $ch = curl_init($api . '/api/gifts/update');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_gift'])) {
    ...
    if (!empty($codes)) {
        $ch = curl_init($api . '/api/gifts/delete');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
