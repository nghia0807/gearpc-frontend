...
$pageSize = 10;
$apiUrl = "http://phpbe_app_service:5000/api/gifts/get?pageIndex=$pageIndex&pageSize=$pageSize";
$alerts = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_gift'])) {
    ...
    if ($code === '' || $name === '') {
        ...
        $ch = curl_init('http://phpbe_app_service:5000/api/gifts/add');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_gift'])) {
    ...
    if ($id === '' || $name === '') {
        ...
        $ch = curl_init('http://phpbe_app_service:5000/api/gifts/update');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_gift'])) {
    ...
    if (!empty($codes)) {
        $ch = curl_init('http://phpbe_app_service:5000/api/gifts/delete');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
